...
void __kernel_push_down(int node) {
    ...
    if (cnt.set == 1) {
        ...
        kernel_heap_nodes[R(node)].max_space =kernel_heap_nodes[R(node)].left_boarder_space =kernel_heap_nodes[R(node)].right_boarder_space = 0;
    } else if (cnt.set == -1) {
        kernel_heap_nodes[L(node)].set = cnt.set;
        kernel_heap_nodes[L(node)].max_space =kernel_heap_nodes[L(node)].left_boarder_space =kernel_heap_nodes[L(node)].right_boarder_space =
                kernel_heap_nodes[L(node)].r-kernel_heap_nodes[L(node)].l+1;
        kernel_heap_nodes[R(node)].set = cnt.set;
        kernel_heap_nodes[R(node)].max_space =kernel_heap_nodes[R(node)].left_boarder_space =kernel_heap_nodes[R(node)].right_boarder_space =
