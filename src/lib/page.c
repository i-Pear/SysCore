...
__Memory_SegmentTreeNode my_pages[__page_num * 4];
int __memory_alloc_length[__page_num];

#define cnt my_pages[node]

int get_size(int node){
    return my_pages[node].r-my_pages[node].l+1;
}

bool isEmpty(int node){
    return my_pages[node].max_space==get_size(node);
}

void __memory_push_up(int node) {
    if(!isEmpty(L(node))){
        cnt.left_boarder_space = my_pages[L(node)].left_boarder_space;
    }else{
        cnt.left_boarder_space = get_size(L(node))+my_pages[R(node)].left_boarder_space;
    }
    cnt.left_right_space = my_pages[L(node)].right_boarder_space;
    cnt.right_left_space = my_pages[R(node)].left_boarder_space;
    if(!isEmpty(R(node))){
        cnt.right_boarder_space = my_pages[R(node)].right_boarder_space;
    }else{
        cnt.right_boarder_space = my_pages[L(node)].right_boarder_space+get_size(R(node));
    }

    cnt.left_max_space = my_pages[L(node)].max_space;
    cnt.right_max_space = my_pages[R(node)].max_space;
...
void __memory_init_pages(int node, int l, int r) {
    ...
    cnt.right_max_space =cnt.right_boarder_space = cnt.right_left_space = my_pages[R(node)].max_space;
    cnt.left_max_space =cnt.left_right_space = cnt.left_boarder_space = my_pages[L(node)].max_space;
}
...
void __memory_push_down(int node) {
    if (cnt.set == 1) {
        my_pages[L(node)].set = cnt.set;
        my_pages[L(node)].max_space = my_pages[L(node)].left_boarder_space = my_pages[L(node)].right_boarder_space = 0;
        my_pages[R(node)].set = cnt.set;
        my_pages[R(node)].max_space = my_pages[R(node)].left_boarder_space = my_pages[R(node)].right_boarder_space = 0;
    } else if (cnt.set == -1) {
        my_pages[L(node)].set = cnt.set;
        my_pages[L(node)].max_space = my_pages[L(node)].left_boarder_space = my_pages[L(node)].right_boarder_space =
                my_pages[L(node)].r - my_pages[L(node)].l + 1;
        my_pages[R(node)].set = cnt.set;
        my_pages[R(node)].max_space = my_pages[R(node)].left_boarder_space = my_pages[R(node)].right_boarder_space =
                my_pages[R(node)].r - my_pages[R(node)].l + 1;
    }
    cnt.set = 0;
}
...
void init_memory(){
    ...
    printf("[Memory] total my_pages' count is %d\n",page_count);
