...
//        releasesleep(&sdcard_lock);
        printf("[panic!] sdcard: fail to read\n");
    }
    ...
    if (0 == timeout) {
        printf("[panic!] sdcard: timeout waiting for reading\n");
    }
...
//        releasesleep(&sdcard_lock);
        printf("[panic!] sdcard: fail to write\n");
    }
    ...
    if (0 == timeout) {
//        releasesleep(&sdcard_lock);
        printf("[panic!] sdcard: invalid response token\n");
    }
    ...
    if (0 == timeout) {
//        releasesleep(&sdcard_lock);
        printf("[panic!] sdcard: timeout waiting for response\n");
    }
    sd_end_cmd();
...
//        releasesleep(&sdcard_lock);
        printf("result: %x\n", result);
        printf("error_code: %x\n", error_code);
        printf("[panic!] sdcard: an error occurs when writing\n");
    }
