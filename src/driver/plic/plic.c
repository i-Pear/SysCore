...
// unsigned long register_read_mhartid(){
//   unsigned long x;
//   asm volatile("csrr %0, mhartid" : "=r" (x) );
//   return x;
// }

#define current_coreid() 0
...
int plic_irq_enable(plic_irq_t irq_number) {
  /* Check parameters */
  if (PLIC_NUM_SOURCES < irq_number || 0 > irq_number) return -1;
  unsigned long core_id = current_coreid();
...
//   /* Check parameters */
//   if (PLIC_NUM_SOURCES < irq_number || 0 > irq_number) return 0;
...
void plic_irq_register(plic_irq_t irq, plic_irq_callback_t callback,
                       void *ctx) {
  /* Read core id */
  unsigned long core_id = current_coreid();
...
// plic_instance_t (*plic_get_instance(void))[IRQN_MAX] { return plic_instance; }
