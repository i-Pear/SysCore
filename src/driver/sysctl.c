...
int sysctl_dma_select(sysctl_dma_channel_t channel, sysctl_dma_select_t select)
{
    ...
    dma_sel0 = (volatile sysctl_dma_sel0_t)sysctl->dma_sel0;
    dma_sel1 = (volatile sysctl_dma_sel1_t)sysctl->dma_sel1;
    switch(channel)
    {
...
uint32 sysctl_pll_get_freq(sysctl_pll_t pll)
{
     ...
     * FOUT = FIN / NR * NF / OD
     */
    freq_out = (uint64)freq_in * 1000 / (uint64)nr * 1000 * (uint64)nf / (uint64)od / 1000000;
    return freq_out;
}
