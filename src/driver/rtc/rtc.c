...
int rtc_timer_set_mode(rtc_timer_mode_t timer_mode) {
  ...
  unsigned long start_cycle = register_read_mcycle();
  // printf("5\n");
  /* Wait for 1/26000000 s to sync data */
  while (register_read_mcycle() - start_cycle < freq) continue;
  // printf("6\n");
...
struct tm *rtc_timer_get_tm(void) {
  if (rtc_timer_get_mode() != RTC_TIMER_RUNNING) return NULL;
...
int rtc_timer_set(int year, int month, int day, int hour, int minute,
                  int second) {
  struct tm date_time = {
      .tm_sec = second,
      .tm_min = minute,
...
int rtc_timer_get(int *year, int *month, int *day, int *hour, int *minute,
                  int *second) {
...
int rtc_alarm_set(int year, int month, int day, int hour, int minute,
                  int second) {
  struct tm date_time = {
      .tm_sec = second,
      .tm_min = minute,
...
int rtc_alarm_get(int *year, int *month, int *day, int *hour, int *minute,
                  int *second) {
...
tick:
  if (instance->tick_enable_by_user) {
    switch (instance->tick_mode_by_user) {
      case RTC_INT_MINUTE:
        if (now_tm->tm_sec != 0) goto ret;
        break;
      case RTC_INT_HOUR:
        if (now_tm->tm_sec != 0 || now_tm->tm_min != 0) goto ret;
        break;
      case RTC_INT_DAY:
...
int rtc_tick_irq_register(bool is_single_shot, rtc_tick_interrupt_mode_t mode,
  ...
  if (!rtc_instance.tick_enable_by_alarm ||
      (rtc_instance.tick_enable_by_alarm &&
       mode < rtc_tick_get_interrupt_mode())) {
...
int rtc_alarm_irq_register(bool is_single_shot, rtc_mask_t mask,
                           plic_irq_callback_t callback, void *ctx,
                           uint8_t priority) {
  plic_irq_disable(IRQN_RTC_INTERRUPT);
...
alarm_mode:
  if ((rtc_instance.tick_enable_by_user &&
       rtc_instance.tick_mode_by_alarm < rtc_tick_get_interrupt_mode()) ||
      !rtc_instance.tick_enable_by_user) {
