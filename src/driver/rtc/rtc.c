...
{
    // plic_irq_disable(IRQN_RTC_INTERRUPT); // TODO 暂时不需要
    rtc_tick_set_interrupt(0);
    ...
    // plic_set_priority(IRQN_RTC_INTERRUPT, priority); // TODO暂时不需要
    // plic_irq_register(IRQN_RTC_INTERRUPT, rtc_irq_callback, &rtc_instance); // TODO暂时不需要
    // plic_irq_enable(IRQN_RTC_INTERRUPT); // TODO暂时不需要
...
void rtc_tick_irq_unregister(void)
{
    ...
    {
        // plic_irq_unregister(IRQN_RTC_INTERRUPT); // TODO 暂时不需要
    }
}
...
int rtc_alarm_irq_register(bool is_single_shot, rtc_mask_t mask, plic_irq_callback_t callback, void *ctx, uint8_t priority)
{
    ...
    // plic_set_priority(IRQN_RTC_INTERRUPT, priority); // TODO 暂时不需要
    // plic_irq_register(IRQN_RTC_INTERRUPT, rtc_irq_callback, &rtc_instance); // TODO暂时不需要
    // plic_irq_enable(IRQN_RTC_INTERRUPT); // TODO 暂时不需要
